<template>
  <teleport to="body">
    <div v-if="show" :id="id" :style="{ zIndex: zIndexValue }">
      <div class="bg-black bg-opacity-50 fixed inset-0" :style="{ zIndex: zIndexValue+1 }" />
      <div
        ref="modalRef"
      ...
      >
        ...
        >
          <!-- Modal content -->
          <div class="relative bg-[#141313] rounded-lg shadow" :class="{'mt-10': !popup}">

            <!-- Modal header -->
            <div
              :class="[$slots.header ? ' border-secondary-bold' : '', popup || !showHeader?'':'border-b p-4']"
            ...
            <!-- Modal body -->
              <div :class="$slots.header ? '' : 'pt-0'"
                class="p-2 pr-1 max-h-[75vh] overflow-y-hidden"
              >
                <slot name="body" />
              </div>
