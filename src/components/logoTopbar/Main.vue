<template>
    <!-- logo-topbar -->
    <div class="logo-topbar flex justify-between items-center space-x-4 relative p-2 pb-1">

        <RouterLink to="/" class="w-[228px] h-16 top-logo flex items-center justify-center xl:justify-start rounded-lg overflow-hidden">
            <logoComp />
            <!-- <img v-if="imgName" :src="'/images/logo/' + imgName + '.png'" @load="handleImageLoad" @error="handleImageError" class="w-[228px] h-16" :class="{ 'hidden': !imgLoad }" />
            <div v-if="!imgLoad" class="flex h-16 w-full font-bold justify-center items-center mt-[-5px] text-3xl text-gray-600 dark:text-tableText">
                {{ domainName }}
            </div> -->
        </RouterLink>
        ...
        <div class="flex items-center">
            ...
            <div class="cursor-pointer ml-2 hover:bg-third-bold rounded-lg transition duration-200 dark:text-tableText" :class="{ 'rotate-0': !toggle, 'rotate-[180deg]': toggle }">
                <MenuIcon v-if="!toggle" class="text-white m-2 w-7 h-7 transition transition-delay-300 hover:scale-110"  @click="toggle = !toggle, mobileToggle = !mobileToggle"/>
                <CloseIcon v-else @click="toggle = !toggle, mobileToggle = !mobileToggle" class="m-2 w-7 h-7 text-white   transition transition-delay-300 hover:scale-110" :class="{ 'rotate-0': toggle, 'rotate-[180deg]': !toggle }"/>
            </div>
        </div>
    ...
    <teleport to="body">
        <div class="visible md:hidden h-full text-black dark:text-tableText bg-secondary">
            <transition name="slide-fade">
                <div v-if="toggle" @click.self="closeLogoTopbar" class="fixed h-full w-full menuBar overflow-y-hidden top-0 left-0"  :style="{ zIndex: zIndexValue+2 }">
                    <div ref="logoTopbar"  class="bg-[#141313] overflow-y-auto overflow-x-hidden h-full menuBar-shadow w-[250px] transform" :class="{ 'translate-x-0': toggle, '-translate-x-full': !toggle }">
                        ...
                        <div class="pb-4  ml-1 flex flex-col shadow"> 
                            ...
                            <PopupDropdown :isOpen="showProfileModal" @close="closeProfileModal" dropdownClass ="dropdown-1 bg-secondary top-[100%] left-[8px] w-[220px] cursor-pointer shadow-lg border border-secondary m-1">
                                <template #dropdown-toggle> 
                                    <div class="unselectable flex flex-col p-1 pt-8 pl-2">
                                        <div class="flex items-center cursor-pointer" @click="toggleProfileModal">
                                            <img class="w-12 h-12 rounded-full" :src="images['profile']" alt=""/>
                                            <div class="ml-4">
                                ...
                                <template #dropdown-body class=""> 
                                    <div class="bg-[#272727] p-2">
                                        <div class="font-bold text-xl text-white ">Profile</div>
                                    <div class="text-white font-medium capitalize pb-2">
                                        {{  profile?.name }} <br>
                                        <span class="text-sm lowercase text-white"> {{ profile?.email }} </span>
                                        </div>
                                        <div @click="clickProfile" class="profile-option text-white"><userIcon class="icon-size ml-0" />My Profile</div>
                                        <div @click="clickRefresh" class="profile-option text-white"><ReloadIcon class="icon-size p-[1px] ml-0" />Refresh</div>
                                        <div class="profile-option text-white"><HelpIcon class="icon-size ml-0 text-white" />Help</div>
                                        <!-- <div>{{ profile?.email }}</div>
                                        <div>{{ profile?.mobile }}</div> -->

                                        <div class="profile-option text-white"  @click="logout"><LockIcon class="icon-size ml-0" />Logout</div>
                                    </div>
                            ...
                            <PopupDropdown :isOpen="showNotificationModal" @close="closeNotificationModal" dropdownClass ="text-white dropdown-1 bg-secondary top-[100%] left-[5%] w-[230px] shadow-lg border border-[#272727] bg-[#1E1E1E]">
                                <template #dropdown-toggle>
                                    <div @click="toggleNotificationModal" class="unselectable flex flex-col cursor-pointer p-2 pt-4 pl-2">
                                        <div class = "flex items-center cursor-pointer font-semibold text-lg">
                                            <ActiveAlertIcon class="w-6 h-6 mr-2 text-white" />
                                            <div class="mt-[-4px] text-white"> Notifications </div>
                                            <RightArrowIcon class="w-[14px] h-[14px] inline ml-4 transform rotate-90 transition transition-delay-300 text-white" :class="{ 'rotate-[-90deg]': showNotificationModal, 'rotate-90': !showNotificationModal }" />
                                        </div>
                                    </div>
                                </template>

                                <template #dropdown-body>
                                    <div class="max-h-[320px] text-sm overflow-y-hidden text-gray-600 dark:text-white  text-center p-2">
                                        <div v-if="notifications.length > 0" class="cursor-pointer relative " v-for="notification in notifications" >
                                            ...
                                            <div class="text-white text-left text-wrap text-xs sm:text-sm truncate mt-0.5">
