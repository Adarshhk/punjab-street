<template>
    <tr class="hidden sm:table-row intro-x text-white">
        ...
        {{ `${order.created_at.split('Z')[0].split('T')[1]}` }}
        </td>
        
        <td>
            <div class="flex items-center text-nowrap capitalize px-2 py-4" v-if="order.broker">
    ...
    <div class="intro-x visible sm:hidden relative my-1" :class="{ 'shadow-md dark:shadow-slate-600 ': fullDetails, 'shadow-sm dark:shadow-slate-600': !fullDetails }">
        <div class="px-4" :class="{ 'bg-secondary-dark font-semibold': fullDetails }">
            ...
            <div class="flex items-center text-nowrap pl-2">
                ...
                <!-- time with price  -->
                {{ `${order.created_at.split('Z')[0].split('T')[1]}` }} @ Rs.{{order.status === 'COMPLETE' && (order.order_type === 'MARKET' || order.order_type === 'MKT') ? order.average_price : order.price }} </div>
