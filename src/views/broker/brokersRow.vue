<template>
  <tr class="hidden sm:table-row intro-x text-white">
    <td class="capitalize ">
      <div class="flex items-center text-nowrap px-2 py-4">
        <img class="shadow-none w-5 h-5 mr-1" :src="`${images[broker.broker_name]}`" :alt="broker.broker_name" />
        &nbsp;&nbsp;{{ broker.broker_name }}
      </div>
    </td>
    ...
    <td class="text-center ">
      <a class="flex items-center justify-center mr-3 " @click="tokenGenerate(broker)" href="javascript:;"
        :class="tokendate(broker.broker_token_date) && broker.token_status === 'success' ? 'text-success' : updatedTodayDate(broker.updated_at) && broker.token_status !== 'success' ? 'text-danger' : ''">
        <Tippy class="tooltip flex items-center justify-center"
          :content="updatedTodayDate(broker.updated_at) && broker.message ? broker.message : 'Error'"
          v-if="updatedTodayDate(broker.updated_at) && broker.token_status === 'failed'">
          <span class="flex items-center ">
            <AlertCircleIcon class="w-4 h-4 mr-1" />Connect
          </span>
        </Tippy>
        <Tippy class="tooltip flex items-center"
          :content="tokendate(broker.broker_token_date) && broker.message ? broker.message : 'Connected'"
          v-else-if="tokendate(broker.broker_token_date)">
          <span class="flex items-center">
            <CheckCircleIcon class="w-4 h-4 mr-1 text-success" />Connected
          </span>
        </Tippy>
        <Tippy class="tooltip flex items-center" :content="'Connect'" v-else>
          <span class="flex items-center">
            <Link2Icon class="w-4 h-4 mr-1" /> Connect
          </span>
        </Tippy>
      </a>
    </td>
  ...
  <div class="intro-x visible sm:hidden relative my-1"
    :class="{ 'shadow-md dark:shadow-slate-600 ': fullDetails, 'shadow-sm dark:shadow-slate-600': !fullDetails }">
    <div class="px-4" :class="{ 'bg-secondary-dark font-semibold': fullDetails }">
      <div @click="fullDetails = !fullDetails" class="flex justify-between items-center py-3 text-sm cursor-pointer">
        <div class="flex items-center text-nowrap pl-2">
          ...
          <RightArrowIcon class="w-[12px] h-[12px] inline ml-1 transform rotate-90 transition transition-delay-300"
            :class="{ 'rotate-[-90deg]': fullDetails, 'rotate-90': !fullDetails }" />
        </div>
        <div class="mt-4">

          <ButtonSwitch @change="handleActiveChange()" :id="`${broker.id}is_active`" name="is_active"
    ...
    <div v-if="fullDetails" class="p-2 capitalize bg-secondary-dark">
      <div class="flex flex-col text-sm overflow-hidden bg-secondary border dark:border-slate-600 p-2 pl-4 rounded-lg">
        ...
        <div class="flex items-center">
          ...
          <div class="ml-4">{{ broker.user_name }}</div>
        </div>
        ...
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <a class="flex items-center mr-3 " @click="tokenGenerate(broker)" href="javascript:;"
              :class="tokendate(broker.broker_token_date) && broker.token_status === 'success' ? 'text-success' : updatedTodayDate(broker.updated_at) && broker.token_status !== 'success' ? 'text-danger' : ''">
              <div v-if="updatedTodayDate(broker.updated_at) && broker.token_status === 'failed'"
                class="flex items-center justify-center"
                :content="updatedTodayDate(broker.updated_at) && broker.message ? broker.message : 'Error'">
                <span class="flex ">
                  <AlertCircleIcon class="w-3 h-3 mr-1 mt-1" />Connect
                </span>
              </div>
              <div v-else-if="tokendate(broker.broker_token_date)" class="flex items-center"
                :content="tokendate(broker.broker_token_date) && broker.message ? broker.message : 'Connected'">
                <span class="flex">
                  <CheckCircleIcon class="w-3 h-3 mr-1 text-success mt-1" />Connected
                </span>
              </div>
              <div v-else class="flex items-center" :content="'Connect'">
                <span class="flex ">
                  <Link2Icon class="w-3 h-3 mr-1 mt-1" /> Connect
                </span>
              </div>
            </a>
          </div>
