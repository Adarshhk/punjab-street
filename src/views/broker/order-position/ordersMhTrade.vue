<template>
    <Modal1 size="5xl" :show="showMhtradeModal" >
  
      <template #body>
        <div class="flex px-3 py-2 text-lg font-medium justify-between ">
          <div>Mhtrade ({{ typeofdata }})</div>
          <div class="flex">
              <div
        ...
        <div class="table-container-modal text-nowrap intro-y">
          <table class="-mt-2 dark:text-white" v-if="showOrdersbol">
            <thead>
              <tr class="text-start">
                ...
                <th class="text-center whitespace-nowrap uppercase">Trading Symbol</th>
            ...
            <tbody>
                ...
                >
                  ...
                  <td>{{ order.side }}</td>
                  <td>{{ order.product }} ({{ order.book_type }})</td>
                  <td>{{ order.price }}</td>
                  <td>{{ order.avg_trade_price }}</td>
                  <td>{{ order.quantity_filled }}/{{ order.disclosed_quantity }}</td>
                  <td>{{ order.status }}</td>
              </tr>
          ...
          <table class="w-full table table-report -mt-2 dark:text-white" v-if="showPositionsbol">
            ...
            <tbody>
              ...
              <td>{{ order.realized_mtm }}</td>
              <td class="table-report__action" >
                  <div class="flex justify-center items-center">
                      <a class="flex items-center text-danger whitespace-nowrap" @click="openSqOffModal(order)"  href="javascript:;" v-if="order.net_qty != 0">
  ...
  const showMhtradeModal = ref<boolean>(false)
  watchEffect(() => {
    showMhtradeModal.value = showOrderPositionModal.value && showBrokerName.value === "mhtrade"
  })
  
  // const showMhtradeModal = computed(() => {
  //   return showOrderPositionModal.value && showBrokerName.value === "mhtrade"
  // })
  ...
  function openSqOffModal(posData: any) {
      ...
      brokerPosSqOffData.value.instrument_token = Number(posData.token)
      brokerPosSqOffData.value.quantity = Math.abs(Number(posData.net_qty))
