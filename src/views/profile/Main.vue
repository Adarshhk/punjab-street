<template>
  <div class="mr-2 ml-2 md:ml-0"> 

    <div class="intro-y flex items-center mt-2 ">
      <h2 class="flex flex-wrap text-lg mr-auto p-2">
        <span class="font-medium  capitalize text-white">Welcome {{ profile.name }}, &nbsp;</span>
    ...
    <div>
      <!-- BEGIN: Profile Info -->
      <div class="intro-y shadow box px-5 pt-5 mt-2">
        <div
          class="flex flex-col lg:flex-row border-b border-[#272727] pb-5 -mx-5 "
        >
          <div
          ...
          >
            ...
            <div class="ml-5">
              <div
                class="max-w-24 sm:max-w-40 truncate sm:whitespace-normal font-medium text-lg text-white"
              >
                {{ profile.name }}
              </div>
              <div class="text-[#FAA63E]">{{ profile.user_role }}</div>
            </div>
          </div>
          <div
          ...
          >
            <div class="flex justify-center lg:justify-start">
              ...
              >
                <div class="text-white font-medium flex justify-center lg:justify-start lg:mt-3 mb-2">
                  Contact Details
                </div>
                <div class="truncate sm:whitespace-normal flex items-center text-white">
                  <MailIcon class="w-4 h-4 mr-2" />
                  {{ profile.email }}
                </div>
                <div class="text-white truncate sm:whitespace-normal flex items-center mt-3">
      ...
      <div class="md:hidden w-full sm:w-60">
        <select
          ...
          class="border border-[#272727] text-sm  block w-full p-2.5"
        >
        ...
        >
          <div v-for="selectedTab in selectItmesList" :key="selectedTab.name" :fullWidth="false" :class="['text-white py-4', 'flex', 'items-center', 'cursor-pointer', selectedwindow === selectedTab.name ? 'border-b border-[#FAA63E]' : '', selectedwindow !== selectedTab.name ? 'deactived' : '']" @click="setselectedwindow(selectedTab.name)">
