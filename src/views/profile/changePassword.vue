<template>
  <div class="intro-x text-white border border-[#272727] shadow box col-span-12 lg:col-span-6">
      ...
      <form class="validate-form" @submit.prevent="submitChangePassword">
        <div class="ml-4">
          ...
          <template v-if="validate.current_password.$error">
            <div v-for="(error, index) in validate.current_password.$errors" :key="index" class="text-[#FAA63E] mt-2">
              {{ error.$message }}
            </div>
          </template>
        </div>
        <div class="ml-4 mt-2">
          <label for="new_password" class="block mb-2 text-sm font-medium text-white">New Password</label>
          <div class="relative">
          ...
          <template v-if="validate.new_password.$error">
            <div v-for="(error, index) in validate.new_password.$errors" :key="index" class="text-[#FAA63E] mt-2">
              {{ error.$message }}
            </div>
          </template>
        </div>
