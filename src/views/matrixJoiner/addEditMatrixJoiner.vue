<template>
  <Modal1 size="xl" :show="showAddEditModal" @close="closeModel">
    ...
    <template #header>
      <div class="flex flex-wrap items-center font-semibold text-lg sm:text-xl text-white">
        {{ matrixJoiner.id ? `Edit   Joiner Info` : `Subscribe this strategy` }}
      </div>
    </template> 
    ...
    <template #body>
      <form class="p-4 pt-2 validate-form" @submit.prevent="save(matrixJoiner.id || 0)">
        ...
        <div class="input-form mt-3" v-if="!matrixJoiner.id">
          <label for="broker_id" class="text-white form-label w-full flex flex-col sm:flex-row font-bold mb-1">
              Broker
            </label>
        
            <MultiselectDropdown :initialOptions="dropdownOptions" imageShow :selected="handleSelectedBroker" class="p-1 text-white bg-white rounded-md"/>
        ...
        <div class="input-form mt-3">
          <label for="lots" class="form-label w-full flex flex-col text-white mb-1 sm:flex-row"> Lot Size </label>
          <input id="lots" v-model.trim="validate.lots.$model" type="number" name="lots" class="form-control rounded-md p-2"
        ...
        <div class="input-form mt-3">
          <label for="re_entry" class="text-white mb-1  form-label w-full flex flex-col sm:flex-row"> Re Entry </label>
          <input id="re_entry" v-model.trim="validate.re_entry.$model" type="number" name="re_entry" class="form-control rounded-md p-2"
        ...
        <div class="input-form mt-3">
          <label class="text-white form-label w-full flex flex-col sm:flex-row"> Status </label>
        ...
        <div class="flex justify-end items-end md:col-span-2 lg:col-span-3 gap-3">
