<template>
    <tr class="hidden sm:table-row intro-x hover:bg-[#272727] transition-all duration-150">
      ...
      <td class="py-4 px-4 text-white text-center">
        {{ matrixJoiner.lots }}
      </td>
      <td class="py-4 px-4 text-white text-center">
      ...
      <td class="text-center text-nowrap py-4 px-4 text-white">
        {{ matrixJoiner.created_at?matrixJoiner.created_at.replace("T"," ").replace("Z",""): '' }}
      </td>
      <td class="">
        <div class="flex justify-center items-center	">
          <button class="text-[#F9A43C] mr-2 flex items-center" @click="showEdit(matrixJoiner)">
            <EditIcon class="w-4 h-4" />Edit
          </button>
          <button class="text-red-500 delete-btn flex items-center" @click="deleteModal(matrixJoiner.id)">
